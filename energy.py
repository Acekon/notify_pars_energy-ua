...
def remove_empty_element_array(array: list) -> list:
...
def telegram_send_text(chat_id: str, text: str):
    ...
    response = requests.post(
        tg_url, json={"chat_id": chat_id, "parse_mode": "html", "text": text}
    )
...
def telegram_update_message(chat_id: str, message_id, text: str):
    ...
    if response.json().get("ok"):
        logger.info(
            {"chat_id": chat_id, "message_id": message_id, "tg": response.json()}
        )
    else:
        logger.error(
            {"chat_id": chat_id, "message_id": message_id, "tg": response.json()}
...
def telegram_delete_message(chat_id: str, message_id):
    ...
    response = requests.post(
        tg_url, json={"chat_id": chat_id, "message_id": message_id}
    )
    if response.json().get("ok"):
        logger.info(
            {"chat_id": chat_id, "message_id": message_id, "tg": response.json()}
...
def save_message_id_db(message_id: int, queue: int):
    ...
    c = conn.cursor()
    sql_query = (
        f'UPDATE energy SET message_id = "{message_id}" WHERE queue = "{queue}";'
    )
    c.execute(sql_query)
    conn.commit()
    conn.close()
...
def get_schedules(queue: int):
    ...
    formated_now_day = "\n".join(
        remove_empty_element_array(schedules[0].text.strip().split("\n"))
    )
    formated_next_day = "\n".join(
        remove_empty_element_array(schedules[1].text.strip().split("\n"))
    )
...
def main():
    day = [i for i in range(7, 20)]  # send only in number hours
    ...
    for i in range(1, 7):  # count queue
        ...
        if site_next_day != db_next_day and current_time in night and (site_next_day.find("ще очікуються") == -1):
            ...
            message_id = telegram_send_text(text=site_next_day, chat_id=CHANNELS.get(i))
